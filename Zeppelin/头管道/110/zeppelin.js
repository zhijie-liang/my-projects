...
for (var i = 0; i < records.length; i++) {
  try {
    ...
    obj.FAIL_INFO = state.convertNullToStr(records[i].value['FAIL_INFO']);
    ...
    obj.RECORD_TIME = records[i].value['RECORDTIME_MYSQLRPAIR'];
    ...
    obj.SOURCE_TYPE = state.convertNullToStr(records[i].value['SOURCE_TYPE']);
    obj.SQL = records[i].value['SQL'];
    obj.SEARCH_RESULT = state.convertNullToStr(records[i].value['SEARCH_RESULT']);
    obj.SQL_TYPE = state.convertNullToStr(records[i].value['SQL_TYPE']);
    obj.START_EXEC_TIME = records[i].value['START_EXEC_TIME'];
    obj.TABLE_NAME = state.convertNullToStr(records[i].value['TABLE_NAME']);
    ...
    obj1.count = 3;
    obj1.ts = timestamp;
    obj1.id = '6541ebeb3913df4217c4217b:origin';
